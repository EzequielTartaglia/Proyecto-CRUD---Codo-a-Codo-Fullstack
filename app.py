...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:berrykey3@localhost/proyecto"
...
class Usuario(db.Model):  # usuario hereda de db.Model
    """
    Definición de la tabla Usuario en la base de datos.
    La clase Usuario hereda de db.Model.
    ...
    def __init__(self, nombre, mail, imagen):
        ...
        Args:
            ...
            mail (str): e-Mail del Usuario.
            imagen (str): URL o ruta de la imagen del usuario.
        """
        self.nombre = nombre
        self.mail = mail
        self.imagen = imagen
...
# Definición del esquema para la clase usuario
class UsuariosSchema(ma.Schema):
    """
    Esquema de la clase usuario.

    Este esquema define los campos que serán serializados/deserializados
    para la clase Usuarios.
    """
    class Meta:
        fields = ("id", "nombre", "mail", "imagen")

usuario_schema = UsuariosSchema()  # Objeto para serializar/deserializar un usuario
usuarios_schema = UsuariosSchema(many=True)  # Objeto para serializar/deserializar múltiples usuario

'''
Este código define un endpoint que permite obtener todos los usuarios de la base de datos y los devuelve como un JSON en respuesta a una solicitud GET a la ruta /usuarios.
@app.route("/usuarios", methods=["GET"]): Este decorador establece la ruta /usuarios para este endpoint y especifica que solo acepta solicitudes GET.
def get_usuarios(): Esta es la función asociada al endpoint. Se ejecuta cuando se realiza una solicitud GET a la ruta /usuarios.
all_usuarios = usuario.query.all(): Se obtienen todos los registros de la tabla de usuarios mediante la consulta usuario.query.all(). Esto se realiza utilizando el modelo usuario que representa la tabla en la base de datos. El método query.all() heredado de db.Model se utiliza para obtener todos los registros de la tabla.
result = usuarios_schema.dump(all_usuarios): Los registros obtenidos se serializan en formato JSON utilizando el método dump() del objeto usuarios_schema. El método dump() heredado de ma.Schema se utiliza para convertir los objetos Python en representaciones JSON.
...
def get_Usuarios():
    ...
    Retorna un JSON con todos los registros de la tabla de usuarios.
    """
    all_usuarios = Usuario.query.all()  # Obtiene todos los registros de la tabla de usuarios
...
'''
El código que sigue a continuación termina de resolver la API de gestión de usuarios, a continuación se destaca los principales detalles de cada endpoint, incluyendo su funcionalidad y el tipo de respuesta que se espera.
Endpoints de la API de gestión de usuarios:
get_usuario(id):
...
delete_usuario(id):
    ...
    # Retorna un JSON con el registro eliminado del usuario correspondiente al ID proporcionado
create_usuario():
    # Crea un nuevo usuario en la base de datos
    # Lee los datos proporcionados en formato JSON por el cliente y crea un nuevo registro de usuario
    # Retorna un JSON con el nuevo usuario creado
update_usuario(id):
...
def get_usuario(id):
    ...
    Retorna un JSON con la información del usuario correspondiente al ID proporcionado.
    """
    usuario = Usuario.query.get(id)  # Obtiene el usuario correspondiente al ID recibido
...
@app.route("/usuarios", methods=["POST"])  # Endpoint para crear un usuario
def create_usuario():
    """
    ...
    Retorna un JSON con el nuevo usuario creado.
    """
    nombre = request.json["nombre"]  # Obtiene el nombre del usuario del JSON proporcionado
    mail = request.json["mail"]  # Obtiene el stock del usuario del JSON proporcionado
    imagen = request.json["imagen"]  # Obtiene la imagen del usuario del JSON proporcionado
    new_usuario = Usuario(nombre, mail, imagen)  # Crea un nuevo objeto usuario con los datos proporcionados
...
def update_usuario(id):
    ...
    Retorna un JSON con el usuario actualizado.
    """
    usuario = Usuario.query.get(id)  # Obtiene el usuario existente con el ID especificado
